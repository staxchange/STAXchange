...
export async function POST(request: Request) {
  ...
  if (!rateLimit.allowed) {
    return NextResponse.json(
      { ok: false, error: "Too many upload requests.", retryAfterSeconds: rateLimit.retryAfterSeconds },
      { status: 429 }
    );
  }
  ...
  try {
    ...
    const upload = await createServiceUploadSignedUrl(supabase, {
      bucket: process.env.SERVICE_ATTACHMENTS_BUCKET ?? process.env.SERVICE_UPLOAD_BUCKET ?? "service-attachments",
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Unknown upload URL error." },
      { status: 400 }
    );
  }
}
